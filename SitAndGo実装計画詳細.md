...
### 8.1 単体テスト項目
- [x] 倍率抽選が確率通りか（10万回シミュレーション） → 誤差0.19%以内で正常
- [x] 期待値 ≈ 4.0 の確認 → 3.99x（99.75% RTP）正常
- [x] 賞金計算が正しいか（全倍率パターン） → 70/30と60/30/10正常
- [x] レーティング計算が正しいか → ゼロサム性確認済み
- [x] 初心者保護（10戦未満・4位）が機能しているか → 50%軽減確認済み
- [x] レート下限0が守られているか → コード実装確認済み
- [x] K-factorがスタック深度に応じて変化するか → 20-100BB対応確認済み

### 8.2 結合テスト項目（1アカウントテスト方法）
#### オプション1: デバッグモード追加
- [ ] `/sng debug start <倍率>` コマンド実装（1人で即開始、NPCダミー3人自動生成）
- [ ] ダミープレイヤーは自動でランダムアクション実行
- [ ] 倍率を手動指定可能にして全パターンテスト

#### オプション2: 最小人数変更
- [ ] config.ymlに `sitandgo.debugMode: true` 追加
- [ ] デバッグモード時は1人で開始可能
- [ ] 不足分はNPC補完

#### オプション3: テスト用コマンド
- [ ] `/sng test-multiplier <倍率>` - 倍率固定で開始
- [ ] `/sng test-rating` - レート計算のみ単体実行
- [ ] `/sng test-prize <倍率> <buyIn>` - 賞金計算のみ単体実行

#### 結合テスト実施項目
- [ ] 4人揃うまで開始しない（または1人デバッグモード）
- [ ] ルーレット演出の動作確認（全倍率パターン）
- [ ] ブラインドレベルが3分ごとに上がることの確認
- [ ] GUI情報表示（時間、ブラインド、レート、倍率）の確認
- [ ] 脱落順位が正しく記録されることの確認
...
**最も簡単な方法: デバッグコマンド実装**
```kotlin
// SitAndGo_Command.kt に追加
if (args[0].equals("debug", true) && player.isOp) {
    val mult = args.getOrNull(1)?.toDoubleOrNull() ?: pickMultiplier()
    val game = SitAndGo(player, 1000)
    game.multiplier = mult
    // ダミープレイヤー3人追加
    repeat(3) { i ->
        val dummy = createDummyPlayer("TestBot${i+1}")
        game.addSitAndGoPlayer(dummy)
    }
    game.start()
}
```
...
**ダミープレイヤー実装**
- ランダムでコール/フォールド/オールイン
- GUIは表示せず内部処理のみ
- タイムアウトなし即座に判断

### チェックリスト
- [x] 8.1 シミュレーションコード作成（JUnit5テスト）
- [x] 8.2 期待値検証実行（3.99x確認済み）
- [ ] 8.3 デバッグモード実装（1人テスト用）
- [ ] 8.4 全倍率での賞金配分テスト（実機）
- [ ] 8.5 レーティング変動テスト（実機）
- [ ] 8.6 タイムバンクテスト（実機）
- [ ] 8.7 ルーレット演出確認（実機）
...
- [x] 順位確定ロジック
- [x] **run() TexasHoldem統合**（親クラスのrun()呼び出し・オーバーライド調整）
- [x] endTournament() 終了処理
- [x] **saveTournamentLog() ログ保存処理**

### フェーズ4: イベント
- [x] SitAndGo_Event.kt 作成
- [x] **GUIクリック処理**（アクションボタン処理の実装）
- [x] 切断時の自動フォールド
- [x] Main.kt イベント登録
...
- [x] Main.kt コマンド登録
- [x] TabCompleter
- [x] **プレイヤーリスト管理**（参加時のplayerList追加・離脱時の削除）
